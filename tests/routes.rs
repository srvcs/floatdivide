...
use srvcs_floatdivide::{api::Deps, health, router, telemetry};
...
/// decides whether `value` is a JSON number, returning the real verdict (a JSON
/// number echoed back plus the boolean result). This genuinely exercises
/// floatdivide's per-operand validation branch rather than rubber-stamping.
...
#[tokio::test]
async fn divides_seven_by_two_to_three_point_five() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(7), json!(2)).await;
    assert_eq!(status, StatusCode::OK);
    assert!((result_f64(&body) - 3.5).abs() < 1e-9);
    assert_eq!(body["a"], json!(7));
    assert_eq!(body["b"], json!(2));
}

#[tokio::test]
async fn divides_floats_approximately() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(1.0), json!(4.0)).await;
    assert_eq!(status, StatusCode::OK);
    assert!((result_f64(&body) - 0.25).abs() < 1e-9);
}

#[tokio::test]
async fn divides_with_a_repeating_quotient() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(1), json!(3)).await;
    assert_eq!(status, StatusCode::OK);
    // 1/3 has no exact f64 representation; compare approximately.
    assert!((result_f64(&body) - 0.333_333_333_333_333_3).abs() < 1e-9);
}

#[tokio::test]
async fn division_by_zero_is_unprocessable() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(7), json!(0)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
    assert_eq!(body["error"], "division by zero");
}

#[tokio::test]
async fn division_by_zero_float_is_unprocessable() {
    let isnumber = spawn_isnumber().await;
    let (status, body) = eval(&isnumber, json!(1.5), json!(0.0)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
    assert_eq!(body["error"], "division by zero");
}

#[tokio::test]
async fn rejects_non_number_operand() {
...
async fn rejects_when_isnumber_says_false() {
    ...
    let (status, _) = eval(&isnumber, json!(7), json!(2)).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
}

#[tokio::test]
async fn degrades_when_isnumber_is_unreachable() {
    let (status, body) = eval(DEAD_URL, json!(7), json!(2)).await;
